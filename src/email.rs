...
impl<'a> TryFrom<&'a [u8]> for Email<'a> {
    ...
    fn try_from(bytes: &'a [u8]) -> Result<Email, Self::Error> {
        let mut dkim_header = None;
        let (headers, body) = parse_message_with_separators(bytes)
            .map_err(|err| format!("failed to parse email: {:?}", err))?;
...
mod test {
    ...
    fn test_signing_sha256() {
        ...
        let mut mail = Email::try_from(mail.as_bytes()).unwrap();
        ...
        let mail = mail
            ...
            .unwrap();
        let mail = Email::try_from(mail.as_bytes()).unwrap();
